...
def create_feedback(user_id: str, history: dict, score: str, note: str) -> str:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute(""" INSERT INTO dbo.FeedBack (FeedbackID, UserID, HistoryID, Question, Answer, Score, Note, CreatedAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?) """,
...
def list_feedback(user_id: str) -> list[dict]:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute("""SELECT FeedbackID, HistoryID, Question, Answer, Score, Note, CreatedAt FROM dbo.FeedBack WHERE UserID = ? ORDER BY CreatedAt DESC """,user_id,)
...
def latest_negative_feedback(user_id: str, limit: int = 3) -> list[dict]:
    ...
    try:
        cursor = conn.cursor()
        cursor.execute( """ SELECT TOP (?) FeedbackID, HistoryID, Question, Answer, Score, Note, CreatedAt FROM dbo.FeedBack WHERE UserID = ? AND Score = 'bad' ORDER BY CreatedAt DESC """,limit, user_id,)
        return [_feedback_row(row) for row in cursor.fetchall()]
    finally:
        conn.close()
